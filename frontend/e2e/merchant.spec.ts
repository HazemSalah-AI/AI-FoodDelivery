...
test('merchant manages products and business availability',async({page})=>{
 ...
 await expect(page.getByRole('heading',{name:'متجرك في نظرة'})).toBeVisible();
 ...
 await page.getByRole('button',{name:'إدارة المنتجات',exact:true}).click();
 await page.getByRole('button',{name:'إضافة منتج'}).click();
 await page.getByLabel('اسم المنتج',{exact:true}).fill('منتج اختبار المتجر');
 ...
 await page.getByRole('button',{name:'إعدادات المتجر',exact:true}).click();
 await page.getByLabel('المتجر مفتوح لاستقبال طلبات جديدة').uncheck();
 await page.getByRole('button',{name:'حفظ',exact:true}).click();
 await expect(page.getByText('المتجر مغلق حاليًا',{exact:true})).toBeVisible();
 await page.getByLabel('المتجر مفتوح لاستقبال طلبات جديدة').check();
 await page.getByRole('button',{name:'حفظ',exact:true}).click();
 await expect(page.getByText('المتجر يستقبل الطلبات',{exact:true})).toBeVisible();
});
